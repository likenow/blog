...
根据一开始的问题 [Using dispatch_once in method swizzling](https://stackoverflow.com/questions/29435788/using-dispatch-once-in-method-swizzling) 中所述，`dispatch_once`  在 `+load`中不是必须的。但是复杂场景下，加上会确保执只行一次。
