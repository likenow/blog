...
- (void)aniCircle {    
    // start path
    UIBezierPath *maskFinalBP =  [UIBezierPath bezierPathWithOvalInRect:self.floatBtn.frame];
    CGPoint finalPoint = CGPointMake(self.floatBtn.center.x, self.floatBtn.center.y);
    // 半径 
    CGFloat radius = sqrt((finalPoint.x * finalPoint.x) + (finalPoint.y * finalPoint.y));
    // end path
    UIBezierPath *maskStartBP = [UIBezierPath bezierPathWithOvalInRect:CGRectInset(self.floatBtn.frame, -radius, -radius)];
    ...
    [maskLayerAnimation setValue:@"splashIconAnimation" forKey: @"splashIconPath"];
    maskLayerAnimation.fromValue = (__bridge id)(maskStartBP.CGPath);
    maskLayerAnimation.toValue = (__bridge id)((maskFinalBP.CGPath));
    maskLayerAnimation.duration = 1;
    maskLayerAnimation.delegate = self;
    maskLayerAnimation.timingFunction = [CAMediaTimingFunction  functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
    [maskLayer addAnimation:maskLayerAnimation forKey:@"splashIconPath"];
}

/// 长方形
- (void)aniRect {
    // start path
    UIBezierPath *maskFinalBP = [UIBezierPath bezierPathWithRect:self.floatBtn.frame];
    // end path
    UIBezierPath *maskStartBP = [UIBezierPath bezierPathWithRect:self.blueView.frame];
    // Create the shape layer to display and animate the line.
    CAShapeLayer *maskLayer = [CAShapeLayer layer];
    // 将它的 path 指定为最终的 path 来避免在动画完成后会回弹
    maskLayer.path = maskFinalBP.CGPath;
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    if (flag) {
        if ([[anim valueForKey:@"splashIconPath"] isEqualToString:@"splashIconAnimation"]) {
//            self.blueView.layer.mask = nil;
...
- (UIButton *)floatBtn {
    if (!_floatBtn) {
        ...
        [_floatBtn setBackgroundColor:[UIColor redColor]];
    }
    return _floatBtn;
}
```
